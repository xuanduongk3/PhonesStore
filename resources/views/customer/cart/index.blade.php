...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2">
        @forelse ($cartItems as $item)
            ...
            @endphp
            <div class="bg-white rounded-lg shadow-md mb-6 overflow-hidden transition duration-300 hover:shadow-lg" data-item data-price = {{ $price}} data-discount = {{ $discount }}>
                <div class="flex flex-col md:flex-row p-6">
                    <div class="flex-shrink-0 mb-4 md:mb-0 md:mr-6">
                        <a href="{{ route('customer.product.detail', ['slug' => $item->productVariant->product->slug]) }}">
                    ...
                    <div class="flex-grow">
                        <div class="flex flex-col md:flex-row md:justify-between">
                            ...
                            <div class="mt-2 md:mt-0 text-right">
                                ...
                                @if ($discount > 0)
                                    <p class="text-sm text-gray-500 line-through data-original">{{ number_format($price) }}₫</p>
                                    <p class="text-lg font-bold text-red-600" data-total>{{ number_format($priceAfterDiscount) }}₫</p>
                                @else
                                    <p class="text-lg font-bold text-red-600" data-total>{{ number_format($price) }}₫</p>
                                @endif
                            </div>
                        ...
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between mt-4">
                            <div class="inline-flex items-center rounded-md overflow-hidden">
                                <button type="button" onclick="decrementQuantity({{ $item->id }})" 
                                    class="px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-600 transition duration-300 cursor-pointer">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" />
                                    </svg>
                                </button>
                                <input type="number" id="quantity-{{ $item->id }}" data-quantity name="quantity[{{ $item->id }}]" value="{{ $item->quantity }}" min="1" 
                            ...
                            <form method="POST" action="{{ route('cart.remove', $item->product_variant_id) }}" class="inline">
                                @csrf
                                <button type="submit" class="mt-3 md:mt-0 inline-flex items-center text-gray-500 hover:text-red-600 transition duration-300 cursor-pointer">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 h-fit sticky top-4">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-center">
                    ...
                    <span id="summary-total" class="font-medium">{{ number_format($totalPrice) }}₫</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">Tổng giảm giá:</span>
                    <span id="summary-discount" class="font-medium text-green-600">-{{ number_format($totalDiscount) }}₫</span>
                </div>
                
                <div class="pt-4 border-t border-gray-200">
                    <div class="flex justify-between items-center">
                        <span class="text-lg font-semibold text-gray-800">Tổng thanh toán:</span>
                        <span id="summary-final" class="text-xl font-bold text-red-600">{{ number_format($finalPrice) }}₫</span>
                    </div>
                </div>
...
<script>
    ...
    function updateCartTotals() {
        ...
        document.querySelectorAll('[data-item]').forEach(item => {
            const quantity = parseInt(item.querySelector('[data-quantity]').value);
            const price = parseFloat(item.dataset.price);
            const discountPercent = parseFloat(item.dataset.discount);
            ...
            item.querySelector('[data-total]').textContent = formatPrice(itemFinal);

            // Cập nhật giá gốc nếu có
            const originalEl = item.querySelector('[data-original]');
            if (originalEl) {
                originalEl.textContent = formatPrice(itemTotal);
            }
        });

        document.getElementById('summary-total').textContent = formatPrice(total);
        document.getElementById('summary-discount').textContent = '-' + formatPrice(discount);
        document.getElementById('summary-final').textContent = formatPrice(total - discount);
    }
