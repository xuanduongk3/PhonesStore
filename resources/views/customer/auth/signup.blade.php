...
<div class="container-fuild mx-auto">
    <div class="flex justify-center items-center p-6">
      <div class="bg-white w-full max-w-md rounded-xl shadow-2xl pt-5 pb-3">
        ...
        <div class="px-9 mt-3">
          <form id="form-signup" action="{{ route('signup.post') }}" method="post">
            @csrf
            ...
            <div class="py-1 mb-3">
              <label class="block">
                ...
                <input type="text" name="username"
                  ...
                  value="{{ old('username') }}"
                  placeholder="Nguyễn Xuân Dương" />
                  @if ($errors->has('username'))
                      <span class="text-sm text-red-700 ps-4 mt-1">{{ $errors->first('username') }}</span>
                  @endif
              </label>
            </div>
            <div class="py-1 mb-3">
            ...
            <div class="py-1 mb-3">
                ...
                <div class="relative">
                    ...
                    <span class="absolute inset-y-0 right-3 flex items-center cursor-pointer text-gray-500" onclick="togglePassword('cfpassword', this)">
                    <i class="fa-regular fa-eye"></i>
                    </span>
                </div>
                @if ($errors->has('cfpassword'))
                        <span class="text-sm text-red-700 ps-4 mt-1">{{ $errors->first('cfpassword') }}</span>
                    @endif
                </label>
            </div>
            <div class="bg-purple-500 p-2 text-center rounded-full">
              <button type="submit" class="uppercase font-bold text-white w-full">Đăng ký</button>
            </div>
        <div class="flex justify-center mt-3">
          <span class="text-sm text-gray-400">Bạn đã có tài khoản? <a href="{{ route('login') }}" class="text-sky-500">Đăng nhập ngay</a></span>
        </div>
