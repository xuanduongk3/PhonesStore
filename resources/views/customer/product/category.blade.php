...
<div class="w-full max-w-screen-xl mx-auto px-4 mb-10 bg-white rounded-2xl shadow p-6">
    ...
    <!-- Bộ lọc -->
    <div class="flex mb-4">
        <form id="sort-form" action="{{ isset($brand) 
                ? route('customer.product.category.brand', ['category' => $category->slug, 'brand' => $brand->name]) 
                : route('customer.product.category', ['category' => $category->slug]) 
    ...
    <!-- Thương hiệu -->
    <div class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 xl:grid-cols-12 gap-x-4 gap-y-3 mb-6">
        @foreach ($brands as $brand)
            <div class="w-full border rounded-md p-1 bg-white border-[#EAECF0] transform transition duration-300 hover:scale-105 shadow-sm">
                <a href="{{ route('customer.product.category.brand', ['category' => $category->slug, 'brand' => $brand->name]) }}"
